...
**The iPhone Duo fold effect, on your MacBook lid.** Close the lid and the desktop tilts away from the hinge, softens toward the far edge, and dissolves into shadow, following the hinge angle in real time.
...
4. Click the menu bar icon. The first line should read **Waiting for the lid** and the icon should show your current lid angle. If it still says **Screen Recording permission needed**, quit Lidfold from the same menu and open it again.
...
- **Opening the lid** plays the effect in reverse only if the Mac did not lock while shut. The lock screen sits above everything.
- **A nudge while typing won't trigger it.** The lid has to be closing at a deliberate pace when it crosses 100°.
- **Nothing to tune.** The five numbers that define the look are in `Effect` at the top of `Sources/Lidfold/Preferences.swift` if you build from source.
...
| `FoldShader.swift`, `FoldRenderer.swift` | One Metal pass. The desktop is held fixed in space while the glass rotates about the hinge; an inverse homography maps every pixel back onto it, then a mip-chain disc blur and a shadow gradient follow the distance from the hinge. |
